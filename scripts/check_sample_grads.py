...
def mem_packed_v(mem_params: jnp.ndarray, amdp: AbstractMDP, pi: jnp.ndarray, obs: int, lambda_0: float = 0.):
...
def lambda_discrep_loss(mem_params: jnp.ndarray, amdp: AbstractMDP,
                        lstd_v1: jnp.ndarray, pi: jnp.ndarray,
                        obs: int, lambda_0: float = 0.):
...
def calc_product_rule(mem_params: jnp.ndarray, mem_aug_pi: jnp.ndarray, mem_aug_amdp: AbstractMDP,
                      mem_lstd_v0_unflat: jnp.ndarray, all_om_val_grads: jnp.ndarray, lstd_v1: jnp.ndarray,
                      obs: int):
    ...
    for mem in range(n_mem):
        p_m_given_o, grad_pm = jax.value_and_grad(prob_mem_over_obs)(mem_params, mem_aug_pi, mem_aug_amdp, obs, mem)
...
def check_samples():
    ...
    obs_space_grad_fn = jax.jit(jax.value_and_grad(obs_space_mem_discrep_loss), static_argnames=['value_type', 'q', 'error_type', 'lambda_0', 'lambda_1', 'alpha', 'flip_count_prob'])
    val_grad_fn = jax.jit(jax.grad(mem_obs_val_func), static_argnames=['obs', 'mem'])
    mem_grad_fn = jax.jit(jax.grad(mem_func), static_argnames=['obs', 'action', 'prev_mem', 'next_mem'])

    # DEBUGGING
    jitted_calc_product_rule = jax.jit(calc_product_rule, static_argnames=['obs'])
    mem_packed_v_grad_fn = jax.jit(jax.value_and_grad(mem_packed_v), static_argnames=['obs', 'lambda_0'])
    lambda_discrep_grad_fn = jax.jit(jax.grad(lambda_discrep_loss), static_argnames=['obs', 'lambda_0'])
    ...
    for g in range(n_grad_updates):
        ...
        for o, m in tqdm(list(product(list(range(amdp.n_obs)), list(range(n_mem))))):
            all_om_val_grads = all_om_val_grads.at[o, m].set(val_grad_fn(mem_params, amdp, pi, o, m))
        ...
        if unrolling_steps > 0:
            all_mem_grads = jnp.zeros((n_mem, amdp.n_obs, amdp.n_actions, n_mem) + mem_params.shape)

            print(f"Calculating memory grads for update {g}")
            for o, m in tqdm(list(product(list(range(amdp.n_obs)), list(range(n_mem))))):
                for a, next_m in list(product(list(range(amdp.n_actions)), list(range(n_mem)))):
                    all_mem_grads = all_mem_grads.at[m, o, a, next_m].set(mem_grad_fn(mem_params, o, a, m, next_m))

            all_om_val_grads = calc_all_unrolled_val_grads(mem_params, amdp, pi, T_td, mem_lstd_v0_unflat, all_mem_grads,
                                                           all_om_val_grads, n_unrolls=unrolling_steps)

        mem_probs = softmax(mem_params, axis=-1)

        loss, analytical_mem_grad = obs_space_grad_fn(mem_params, mem_aug_pi, amdp, alpha=1., value_type='v')
